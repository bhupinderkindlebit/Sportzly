    ...
    self.load = function () {
        ...
        scsHdlr = function (result) {
            ...
            if (valid(results)) {
                ...
                {
                    ownerId=results.Results[0].OwnerId;
                }
                if (teamid > 0) {
                    $("#ancAlbum").attr("href", "/Albums.aspx?" + teamid);
                    scsHdlr = function (result) {
                        var results = getJson(result);
                        if (valid(results)) {
                            self.ResultCount = results.Results.length;
                            if (results.Results.length > 0) {
                                $("#ancAlbum").text("Albums (" + self.ResultCount + ")");
                            }
                            else {
                    ...
                    GetAlbums(scsHdlr, errHdlr, getUser().UserKey, "", parseInt(teamid));
                }
                else {
                    scsHdlrCount = function (result) {
                        ...
                        if (valid(results)) {
                            self.ResultCount = results.Results.length;
                            if (results.Results.length > 0) {
                                $("#ancAlbum").attr("href", "/Albums.aspx?" + results.Results[0].EmailId);
                                scsHdlr = function (result) {
                                    var results = getJson(result);
                                    if (valid(results)) {
                                        self.ResultCount = results.Results.length;
                                ...
                                GetAlbums(scsHdlr, errHdlr, getUser().UserKey, results.Results[0].EmailId, getTeam());
                            }                           
                        }
                    }
                    GetEmail(scsHdlrCount, errHdlr, getUser().UserKey, ownerId);
                }
        ...
        var aid = albumId();
        var teamid = teamfunc();
        GetUserPhotosByAlbum(scsHdlr, errHdlr, getUser().UserKey, self.userEmail, parseInt(teamid), parseInt(aid));
        ...
        //    }
        //}
        //GetAlbumCount(scsHdlrCount, errHdlr, getUser().UserKey, self.userEmail, parseInt(teamid));
    ...
    scsHdlr = function (result) {
        ...
        if (valid(results)) {
            ...
            for (var i = 0; i < results.Results.length; i++) {
                ...
                $(".centerPhoto").attr("src", results.Results[i].ImagePath);
            ...
            var teamid = teamfunc();
            GetPhotoComments(scsHdlr, errHdlr, getUser().UserKey, 1, id, parseInt(teamid));
            $(".overlayBox").before('<div id="overlay" class="overlay" style=""></div>');
            $(".overlayBox").data("overlay").load();
...
$(document).ready(function () {
    ...
    $("body").delegate(".leftArrow", "click", function (event) {
        ...
        scsHdlr = function (result) {
            ...
            if (valid(results)) {
                ...
                var teamid = teamfunc();
                //actPages = parseInt(activitiesModel.activities().length);
                GetPhotoComments(scsHdlr, errHdlr, getUser().UserKey, 1, parseInt(photoid), parseInt(teamid));
    ...
    $("body").delegate(".delphoto", "click", function (event) {
        ...
        if (r) {
            ...
            var aid = albumId();
            var teamid = teamfunc();
            GetUserPhotosByAlbum(scsHdlr, errHdlr, getUser().UserKey, getUserEmail(), parseInt(teamid), parseInt(aid));
        }
        else {
            return false;
        }
    ...
    function ShareComments(msg, photoid) {
        ...
        var teamid = teamfunc();
        SaveComment(scsHdlr, errHdlr, getUser().UserKey, msg, parseInt(photoid), parseInt(teamid));
    }
